...
half4 CalculateLight (v2f i)
{
	...
	#define LinearLuminance(color) dot(color, half3(0.2126h, 0.7152h, 0.0722h))

	float fade = fadeDist * unity_LightmapFade.z + unity_LightmapFade.w;
	half3 e = normalize(wpos - _WorldSpaceCameraPos);
	half3 h = normalize(lightDir - e);
	half dotNL = saturate(dot(normal, lightDir));
	half dotNH = saturate(dot(normal, h));
	half sp = exp2(nspec.a * 8.0h + Log2Of1OnLn2_Plus2);
	half d = exp2(sp * dotNH - sp);
  	//half sp = exp2(nspec.a * 8.0h + 2.0h); // [4,1024]
	//half d = pow(dotNH, sp);
	
	// The normalization factor is handled in the material combine phase both
	// to save instructions, and to avoid blowing out the low-precision buffer
	// in edit mode.
	half4 result;
	result.rgb = _LightColor.rgb;
  	result.a = LinearLuminance(_LightColor.rgb) * d; 
